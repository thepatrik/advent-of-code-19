...
fn main() -> Result<(), std::io::Error> {
    ...
    io::stdin().read_to_string(&mut wire_one)?;

    let mut wire_two = String::new();
    io::stdin().read_to_string(&mut wire_two)?;

    let res = part_one(wire_one, wire_two);
    println!("part one res: {}", res.unwrap());

    Ok(())
}

fn part_one(wire_one_input: String, wire_two_input: String) -> Result<i32, std::io::Error> {
    let collect_f = |input: String| -> HashSet<String> {
        ...
        for s in split {
            ...
            match dir {
                "U" => {
                    for i in y..y + distance + 1 {
                        y = i;
                        wire.insert(x.to_string() + ":" + &y.to_string());
                    }
                }
                "D" => {
                    for i in (y - distance..y + 1).rev() {
                        y = i;
                        wire.insert(x.to_string() + ":" + &y.to_string());
                    }
                }
                "L" => {
                    for i in (x - distance..x + 1).rev() {
                        x = i;
                        wire.insert(x.to_string() + ":" + &y.to_string());
                    }
                }
                "R" => {
                    for i in x..x + distance + 1 {
                        x = i;
                        wire.insert(x.to_string() + ":" + &y.to_string());
                    }
                }
    ...
    let wire_one_set = collect_f(wire_one_input);
    let wire_two_set = collect_f(wire_two_input);

    let mut intersects = |s1: HashSet<String>, s2: HashSet<String>| -> Vec<Point> {
        ...
        return matches;
    }(wire_one_set, wire_two_set);

    // Sort intersections
    intersects.sort_by(|a, b| (a.x.abs() + a.y.abs()).cmp(&(b.x.abs() + b.y.abs())));

    // Calculate manhattan distance
    let dist = intersects[1].x.abs() + intersects[1].y.abs();
